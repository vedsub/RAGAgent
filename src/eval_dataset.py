...
Generate exactly {num_queries} queries, one per line. Output ONLY the queries, nothing else."""
    ...
    def _generate_queries_for_chunk(self, chunk: str, num_queries: int = 2) -> List[str]:
        """
        ...
        """
        try:
            prompt = self.query_prompt.format(chunk=chunk[:2000], num_queries=num_queries)
            
            response = self.llm.invoke([
                SystemMessage(content="You are a search query generator. Output only the queries, one per line."),
